...
int main(int argc, char **argv)
{
	...
	findSymbolSectionAndOffset("_prolog", header.prolog_section_index, header.prolog_function_offset);
	findSymbolSectionAndOffset("_epilog", header.epilog_section_index, header.epilog_function_offset);
	findSymbolSectionAndOffset("_unresolved", header.unresolved_section_index, header.unresolved_function_offset);
	...
	const auto tryInsertSymbol = [](std::vector<RSOSymbol>& symbolTable, std::string symbolName, u32 sectionIndex, u32 sectionRelativeOffset) {
        const auto hash = getHash(symbolName);
        const auto it = std::find_if(symbolTable.begin(), symbolTable.end(), [&hash](const RSOSymbol& p) { 
			return p.hash == hash;
		});
		...
		symbolTable.emplace_back(RSOSymbol{hash, std::move(symbolName), sectionIndex, sectionRelativeOffset});
    ...
    for (const auto& section : inputElf.sections) {
        ...
        const auto fromValidSection = std::find_if(cRelSectionMask.begin(), 
			cRelSectionMask.end(),
			[&](const std::string& val) {
		...
		for (auto i = 0u; i < relocations.get_entries_num(); ++i) {
			...
			if (sectionIndex != 0) {
				// Internal Relocation
                auto [symbolIndex, hash] = tryInsertSymbol(internalSymbolTable, std::move(symbolName), rel.section, symbolValue);
                rel.symbolHash = hash;
				...
				// External Relocation
                auto [symbolIndex, hash] = tryInsertSymbol(externalSymbolTable, std::move(symbolName), rel.section, rel.offset);
                rel.symbolHash = hash;
                rel.symbolIndex = symbolIndex;
                rel.addend = 0;
    ...
    for (const auto& relocation : externalRelocations) {
        ...
        const auto& symbolIt = std::find_if(externalSymbolTable.begin(), externalSymbolTable.end(),
			[&](const RSOSymbol& p) {
                return p.hash == relocation.symbolHash;
            });

		if (symbolIt == externalSymbolTable.end()) {
			// This should be imposible since the hash of the symbol come from a function that add the symbol to the list
    ...
    for (auto idx = 0u; idx < externalSymbolTable.size(); ++idx) {
        ...
        const auto& relIt = std::find_if(externalRelocations.begin(), externalRelocations.end(),
            [&](const Relocation& rel) { 
                return rel.symbolHash == externalSymbol.hash;
            });

        if (relIt == externalRelocations.end()) {
            printf("Error! Unable to find relocation for external symbol %s", externalSymbol.symbol.c_str());
            return 1;
        }

...
class Value
{
  public:
    ...
    operator std::filesystem::path() { return str; }
...
class Values
{
  public:
    ...
    bool is_set_by_user(const std::string& d) const { return _user_set.find(d) != _user_set.end(); }
    ...
    std::vector<std::string>& all(const std::string& d) { return _append_map[d]; }

    const std::vector<std::string> all(const std::string& d) const
    {
        static const std::vector<std::string> empty;
        return (_append_map.find(d) == _append_map.end()) ? empty : _append_map.at(d);
    }
...
static std::string& str_replace_helper(std::string& s, const std::string& patt,
...
static OptionParser& add_option_group_helper(OptionParser& parser, const OptionGroup& group);

static Values& parse_args_helper(OptionParser& parser, const std::vector<std::string>& v);
...
class Option
{
  public:
    ...
    {
        _action = a;
        if (a == "store_const" or a == "store_true" or a == "store_false" or a == "append_const" or
            a == "count" or a == "help" or a == "version")
        {
            nargs(0);
        }
        return *this;
    ...
    Option& nargs(size_t n)
    {
        ...
        if (n > 1)
        {
            throw std::invalid_argument("nargs greater than 1 not supported");
        }
  ...
  private:
    ...
    std::string format_option_help(const unsigned int indent = 2) const
    {
        ...
        {
            ss << detail::str_join_trans(", ", _short_opts.begin(), _short_opts.end(),
        ...
        if (not _long_opts.empty())
        {
            ss << detail::str_join_trans(", ", _long_opts.begin(), _long_opts.end(),
    ...
    std::string format_help(const unsigned int indent = 2) const
    {
        ...
        if (help() != "")
        {
            std::string help_str = (get_default() != "") ?
                                       detail::str_replace(help(), "%default", get_default()) :
                                       help();
    ...
    friend OptionParser& detail::add_option_group_helper(OptionParser& parser,
...
class OptionParser
{
  public:
    OptionParser()
        : _usage("%prog [options]"), _add_help_option(true), _add_version_option(true),
    ...
    const std::vector<std::string>& args() const { return _leftover; }

    std::string format_help() const { return detail::format_help_helper(*this); }
    ...
    void print_help() const { std::cout << format_help(); }
    ...
    void print_usage() const { print_usage(std::cout); }

    std::string get_version() const { return detail::str_replace(_version, "%prog", prog()); }

    void print_version(std::ostream& out) const { out << get_version() << std::endl; }

    void print_version() const { print_version(std::cout); }
  ...
  private:
    ...
    const Option& lookup_long_opt(const std::string& opt) const
    {
        ...
        for (std::map<std::string, Option const*>::const_iterator it = _optmap_l.begin();
             it != _optmap_l.end(); ++it)
        {
    ...
    friend Values& detail::parse_args_helper(OptionParser& parser,
...
{
  public:
    OptionGroup(const std::string& t, const std::string& d = "") : _title(t), _group_description(d)
    {
    }
...
static Values& parse_args_helper(OptionParser& parser, const std::vector<std::string>& v)
{
    ...
    for (std::list<Option>::const_iterator it = parser._opts.begin(); it != parser._opts.end();
         ++it)
    {
         ...
         group_it != parser._groups.end(); ++group_it)
    {
        ...
        for (std::list<Option>::const_iterator it = (*group_it)->_opts.begin();
